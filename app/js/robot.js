class Robot {
	...
	{   
		...
		loader.load('obj/boule.DAE', collada => {
			// Init texture rendering
			this.dataTexture = ['obj/face.jpg','obj/texture.jpg'];
	...
	turnOnAnimation()
	{
		...
		TweenMax.to(this.eyelidBottom.rotation, .5,{x: getRadian(-100),ease: Power2.easeOut, onStart: () => {this.parameters.introComplete = true}}, 1.5);
	}
	...
	newGui() 
	{
		...
		f1.add(this.animation, 'flying').onChange(e => {this.standbyAnimation(e)});
 ...
 */
getRandom = (min, max) => Math.random() * (max - min + 1) + min;
